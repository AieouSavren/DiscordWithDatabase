		  ...
		  else
		  {
			  ...
			  if (db) {
				...
				try {
					...
					db.collection("counters").findOneAndUpdate({"_id": "tfinfs"}, {$set: {"seq": num-1}},  function(err,doc) {
					   if (err) { throw err; }
					...
					for(counter = 1; counter < num-1; counter++)
					{
					 ...
					 db.collection("tfinfs").findOneAndUpdate(query, {$set: {"_id": counter}},  function(err,doc) {
					   if (err) { throw err; }
					...
					db.collection("counters").findOneAndUpdate({"_id": "tfspecies"}, {$set: {"seq": num-1}},  function(err,doc) {
					   if (err) { throw err; }
					   else { console.log("Updated Counter"); }
					});
					});
					for(counter = 1; counter < num-1; counter++)
					{
						...
						var query = { "_id": counter}
					 db.collection("tfspecies").findOneAndUpdate(query, {$set: {"_id": counter}},  function(err,doc) {
					   if (err) { throw err; }
					   else { console.log("Updated id:" + counter); }
						});
					}
